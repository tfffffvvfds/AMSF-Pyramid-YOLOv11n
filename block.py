...
class HAFB(nn.Module):
    ...
    def forward(self, inputs):
        if len(inputs) >= 2:
            ...
            raise ValueError("Inputs should contain at least 2 elements.")
        # x1, x2 = inputs
        W_x1 = self.W_x1(x1)
        W_x2 = self.W_x2(x2)
        # 检查尺寸是否一致，如果不一致则进行插值调整
        if W_x1.size(2) != W_x2.size(2) or W_x1.size(3) != W_x2.size(3):
            W_x2 = F.interpolate(W_x2, size=W_x1.size()[2:], mode='bilinear', align_corners=False)
