...
#
# class FeedForward(nn.Module):
#     def __init__(self, dim, ffn_expansion_factor, bias, groups=1):
#         super().__init__()
#         hidden_features = int(dim * ffn_expansion_factor)
#
#         # 保证维度可被分组数整除
#         hidden_features = (hidden_features // groups) * groups
#
#         # 改进的分组膨胀卷积结构
#         self.project_in = nn.Sequential(
#             nn.Conv2d(dim, dim, 3, dilation=2, padding=2,  # 深度卷积替代
#                       groups=dim, bias=bias),  # 减少75%参数
#             nn.Conv2d(dim, hidden_features, 1, groups=groups, bias=bias)  # 分组1x1卷积
#         )
#
#         self.project_out = nn.Sequential(
#             nn.Conv2d(hidden_features, hidden_features, 3, dilation=2, padding=2,
#                       groups=hidden_features, bias=bias),  # 深度卷积
#             nn.Conv2d(hidden_features, dim, 1, groups=groups, bias=bias)
#         )
#
#     def forward(self, x):
...
class DilatedOCA(nn.Module):
    def __init__(self, dim, window_size, overlap_ratio, num_heads, dim_head, bias):
        ...
        # 确保 unfold 的 padding 计算正确
        self.unfold_padding = (self.overlap_win_size - window_size) // 2
    ...
    def forward(self, x):
        ...
        # Step 1: 动态填充输入到 window_size 的整数倍
        pad_h = (self.window_size - h % self.window_size) % self.window_size
        pad_w = (self.window_size - w % self.window_size) % self.window_size
        x = F.pad(x, (0, pad_w, 0, pad_h))  # 右侧和底部填充 (左, 右, 上, 下)
        ...
        # Step 3: 窗口划分（确保 new_h 和 new_w 能被 window_size 整除）
        qs = rearrange(
            qs,
            'b c (h p1) (w p2) -> (b h w) (p1 p2) c',
            p1=self.window_size,
            p2=self.window_size,
            h=new_h // self.window_size,
            w=new_w // self.window_size
        )

        # Step 4: 处理 Key 和 Value
        ks, vs = map(lambda t: self.unfold(t), (ks, vs))
        ks, vs = map(lambda t: rearrange(t, 'b (c j) i -> (b i) j c', c=self.inner_dim), (ks, vs))
